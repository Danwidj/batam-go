...
- Real place photos live in `frontend/public/assets/places/` (bundled, referenced as `/assets/places/<file>`), same pattern as `frontend/public/assets/badges/`. Wikimedia Commons does not have dedicated photos for every named place invented for this app (e.g. no hits for "Dataran Engku Putri" or "Mangrove Boardwalk" despite exhaustive search) — verify a candidate file's actual image content before using it, since filenames can be mismatched or mislabeled (one file titled "Batam, Indonesia (Unsplash).jpg" turned out to be an unrelated portrait photo).
