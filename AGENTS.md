...
- `frontend/src/data/destinations.js` replaced the old `pois.js`/`routes.js` pair: a flat array of `{ poi, routes }` objects. Route `path` arrays are hard-coded, real, road-snapped GeoJSON coordinate arrays — the app never calls a routing API at runtime, it only renders `<Polyline positions={route.path}>`. If regenerating route data, note that the public `router.project-osrm.org` demo server's `/foot/` endpoint silently serves car-profile results (same distance/duration as `/driving/`) rather than real pedestrian routing — use `https://routing.openstreetmap.de/routed-foot` instead, a genuine foot-profile OSRM instance (verify by checking implied walking speed ≈ 1.2–1.4 m/s, not ~9-10 m/s).
- Live location tracking (`frontend/src/hooks/useLiveLocation.js`, continuous `watchPosition` with a Batam-bounds sanity fallback) and marker icons (`frontend/src/utils/mapIcons.js`) are shared between `MapView.jsx` and `Shop.jsx` — don't reintroduce per-component copies.
